...
public class ExpandAllTabView extends LinearLayout implements OnDismissListener {
	...
	public ExpandAllTabView(Context context) {
		super(context);
		init(context);
	}

	public ExpandAllTabView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
		...
		if (position < mToggleButton.size()) {
			switch (position){
				...
				case 1:
					...
					break;
				case 2:
					valueText = valueText.equals("全部")?"适用人群":valueText;
					break;
			}
	...
	public void setValue(ArrayList<String> textArray, ArrayList<View> viewArray) {
		...
		for (int i = 0; i < viewArray.size(); i++) {
			...
			int maxHeight = (int) CommonHelp.dip2px(mContext,285);
			...
			if (i < viewArray.size() - 1) {
				LayoutParams lp = new LayoutParams(2, CommonHelp.dip2px(mContext,14));
